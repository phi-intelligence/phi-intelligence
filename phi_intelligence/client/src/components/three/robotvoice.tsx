...
interface Robot3DProps {
  // Only inactive state needed - no props required
}
...
const Robot3D: React.FC<Robot3DProps> = () => {
  ...
  // Animation state refs - only breathing for inactive state
  ...
  // No animation state changes needed - always inactive
  ...
  useEffect(() => {
    ...
    // Add white accent light for clean glow effect
    const whiteLight = new THREE.PointLight(0xffffff, 0.5, 10);
    whiteLight.position.set(0, 2, 0);
      ...
      (gltf: any) => {
        ...
        robotModel.traverse((child: any) => {
          if (child instanceof THREE.Mesh) {
            ...
            // Apply futuristic grey and white color scheme
            if (child.material) {
              if (Array.isArray(child.material)) {
                child.material.forEach(material => {
                  if (material instanceof THREE.Material) {
                    // Main body: light grey metallic
                    (material as any).color = new THREE.Color(0xcccccc);
                    (material as any).metalness = 0.8;
                    (material as any).roughness = 0.2;
                    (material as any).envMapIntensity = 0.8;
                    ...
                    if (child.name.toLowerCase().includes('eye') || 
                      ...
                      (material as any).emissive = new THREE.Color(0xffffff);
                ...
                // Main body: light grey metallic
                (child.material as any).color = new THREE.Color(0xcccccc);
                (child.material as any).metalness = 0.8;
                (child.material as any).roughness = 0.2;
                (child.material as any).envMapIntensity = 0.8;
                ...
                if (child.name.toLowerCase().includes('eye') || 
                  ...
                  (child.material as any).emissiveIntensity = 0.6;
    ...
    const animate = () => {
      ...
      if (robotRef.current) {
        // INACTIVE STATE ONLY: Static pose with subtle breathing
        ...
        robotRef.current.position.y = -0.5 + Math.sin(breathingOffset.current) * 0.01;
        ...
        // Keep head in natural position (don't force rotation to 0)
        // Head should maintain its natural orientation from the model
      }
