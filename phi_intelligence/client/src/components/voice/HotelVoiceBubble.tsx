...
// Error boundary component for HotelVoiceBubble
class HotelVoiceBubbleErrorBoundary extends React.Component<
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('HotelVoiceBubble Error Boundary caught an error:', error, errorInfo);
  }
      ...
      return (
        <div className="voice-bubble-error-fallback p-6 bg-red-900/20 border border-red-500/30 rounded-lg text-center">
          <div className="text-red-400 mb-3">
            ...
            <h3 className="text-lg font-semibold">Hotel Voice Component Error</h3>
          </div>
          <p className="text-red-300 text-sm mb-4">
            Something went wrong with the hotel voice component. Please refresh the page.
          </p>
          <button
...
const HotelVoiceBubble: React.FC = () => {
  const {
    isConnected, 
    isConnecting, 
    isSpeaking, 
  ...
  useEffect(() => {
    if (agentSwitchTrigger > 0 && currentAgent !== 'hotel' && isConnected) {
      debugLog('🔄 Another agent selected, disconnecting hotel voice bubble...');
      disconnect();
    }
  ...
  const handleConnectionError = useCallback((error: Error, context: string) => {
    console.error(`❌ Hotel Voice ${context} error:`, error);
    setHasError(true);
  ...
  const retryConnection = useCallback(async () => {
    ...
    try {
      ...
      // Attempt to reconnect to hotel
      const roomName = `hotel_reception_room_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      const tokenServerUrl = import.meta.env.VITE_HOTEL_RECEPTIONIST_TOKEN_SERVER_URL;
      ...
      const tokenResponse = await fetch(`${tokenServerUrl}/token`, {
        ...
        body: JSON.stringify({
          room_name: roomName,
          participant_identity: `hotel_guest_${Date.now()}`,
      ...
      if (!tokenResponse.ok) {
        throw new Error(`Hotel token request failed: ${tokenResponse.status}`);
      }
      ...
      setErrorMessage('');
      setRetryCount(0);
      debugLog('✅ Hotel retry connection successful!');
      
    } catch (retryError) {
      const errorObj = retryError instanceof Error ? retryError : new Error('Hotel retry failed');
  ...
  const handleVoiceBubbleClick = async () => {
    debugLog('🎯 HotelVoiceBubble clicked!', { isConnected, isConnecting, mediaDevicesSupported });
    ...
    if (!isConnected) {
      // Connect to hotel room and auto-start voice agent
      debugLog('🔗 Connecting to hotel voice agent...');
      try {
        // Generate hotel room name explicitly
        const roomName = `hotel_reception_room_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        
        // Get Hotel Token Server URL (port 8004)
        const tokenServerUrl = import.meta.env.VITE_HOTEL_RECEPTIONIST_TOKEN_SERVER_URL;
        
        // Get token from Hotel Token Server (port 8004)
        const tokenResponse = await fetch(`${tokenServerUrl}/token`, {
          method: 'POST',
          headers: {
        ...
        if (!tokenResponse.ok) {
          throw new Error(`Failed to get hotel token: ${tokenResponse.status} ${tokenResponse.statusText}`);
        }

        const tokenData = await tokenResponse.json();
        
        // Connect to hotel room - FrameworkWrapper will handle disconnection if needed
        await connect(tokenData.token, roomName, tokenData.server_url);
        
        debugLog('✅ Hotel voice agent connected and ready!');
        
      } catch (error) {
        const errorObj = error instanceof Error ? error : new Error('Unknown hotel connection error');
  ...
  const getStatusText = () => {
    if (hasError) {
      return errorMessage || 'Hotel connection error occurred';
    }
    
    if (isRetrying) {
      return `Retrying hotel connection... (${retryCount}/3)`;
    }
    ...
    if (isConnecting) {
      return 'Connecting to hotel reception...';
    }
    
    if (!isConnected) {
      return 'Tap to speak with our hotel receptionist';
    }
    
    if (!isAgentAvailable || !isAgentAvailable()) {
      return 'Waiting for hotel AI agent to join...';
    }
    
    if (isActive) {
      return 'Hotel receptionist active - tap again to disconnect';
    }
    
    return 'Connected to hotel reception - tap again to disconnect';
  };

  // Get bubble content based on state - HOTEL SYMBOL instead of logo
  const getBubbleContent = () => {
    // Always show hotel symbol - no state-based icon changes
    return (
      <div className="hotel-symbol">
        <Building2 className="w-16 h-16 text-white" />
        <div className="hotel-name text-sm font-medium text-center text-white mt-2">
          Grand Plaza Hotel
        </div>
        <div className="hotel-location text-xs opacity-70 text-center text-white">
          London, UK
        </div>
      </div>
    );
  ...
  const getBubbleClasses = () => {
    let classes = 'hotel-voice-bubble';
    ...
    <HotelVoiceBubbleErrorBoundary>
      ...
      >
        ...
        <div 
          ...
          data-testid="hotel-voice-bubble-clickable"
        >
          {getBubbleContent()}
        ...
        <div className="transcript-container">
          ...
          {response && (
            <div className="response">
              <strong>Receptionist:</strong> {response}
            </div>
          )}
        </div>
      </div>
...
export default HotelVoiceBubble;
